...
%   out.modules(i).skippedRows
    ...
    while true
        ...
        if moduleCount > numel(modulesCell)
            modulesCell = [modulesCell; cell(numel(modulesCell), 1)]; %#ok<AGROW>
        end
        modulesCell{moduleCount, 1} = struct( ...
            'headerLine', line, ...
            'varNames', {varNames}, ...
    ...
    modules = [modulesCell{1:moduleCount}];
